...
1. 食客端默认展示当日和次日完整食谱
2. 午餐/晚餐按三菜一汤标准（3 菜 + 1 汤）
3. 食客可点击增加/删除食谱中的菜品
4. 展现的菜品以有库存优先为通用标准

### 实施

**后端 `lib/mealPlanner.js`**：`buildMeal` 改为三菜一汤
- 菜1：荤菜（避开昨天同蛋白，只选有蛋白的）
- 菜2：素菜（优先 vegPool，回退 otherPool 有蛋白的）
- 菜3：荤素混合（从剩余池加权随机）
- 汤：必选（不再 30% 概率）

**后端 `routes/mealPlan.js`**：新增两个端点
- `POST /api/meal-plan/dish`：向指定天/餐添加一道菜
- `DELETE /api/meal-plan/dish`：从指定天/餐移除一道菜

**前端 `GuestView.vue`**：
- 替换「今日计划摘要」为「今明食谱面板」：今日+明日两个卡片，按早/午/晚/加餐分组
- 每道菜显示菜名、库存绿点、删除按钮（×）
- 每个餐次有「+ 加菜」按钮，弹出选菜弹窗（库存优先排序，最多 100 条）
- 「一键点今日菜」按钮保留
- `fetchRecommendations` count 8→100，让库存优先排序覆盖更多菜谱

### 验证

```
=== 生成周计划 ===
mon lunch: 4 道 → 腊肠烧土豆, 凉拌金针菇, 微波炉版FKC同款嫩牛五方, 罗宋汤
mon dinner: 4 道 → 冬瓜酿肉, 炒滑蛋, 电饭煲排骨土豆焖饭, 玉米排骨汤
